...
namespace AnythingAnywhere.Framework.Patches
{
    internal sealed class PlacementPatches : PatchHelper
    {
        ...
        private static bool PlacementActionPrefix(SObject __instance, GameLocation location, int x, int y, ref bool __result, Farmer? who = null)
        {
            ...
            if (__instance.Category == -19 && location.terrainFeatures.TryGetValue(placementTile, out TerrainFeature? terrainFeature3) && terrainFeature3 is HoeDirt { crop: not null } dirt3 && __instance.QualifiedItemId is "(O)369" or "(O)368" && dirt3.crop.currentPhase.Value != 0)
            {
                return true;
            }
            if (__instance.isSapling())
            {
                    ...
                    ModEntry.Config.EnablePlaceAnywhere)
                {
                    ...
                    Bush newBush = new(placementTile, 3, location);
