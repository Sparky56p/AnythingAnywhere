...
namespace AnythingAnywhere.Framework.Utilities
{
    internal static class ConsoleCommands
    {
        ...
        private static void DebugRemoveFurniture(string command, string[] args)
        {
            ...
            // get target location
            GameLocation? location = Game1.locations.FirstOrDefault(p => p.Name?.Equals(args[0], StringComparison.OrdinalIgnoreCase) == true);
            ...
            // remove objects
            int removed = 0;
            foreach (StardewValley.Objects.Furniture? pair in location.furniture.ToArray())
            {
        ...
        private static void DebugRemoveObjects(string command, string[] args)
        {
            ...
            // remove objects
            int removed = 0;
            foreach ((Microsoft.Xna.Framework.Vector2 tile, SObject obj) in location.Objects.Pairs.ToArray())
            {
        ...
        private static void DebugGreenhouseFix(string command, string[] args)
        {
            ...
            {
                foreach (Building? building in location.buildings.Where(building => building.buildingType.Value == "Greenhouse" && building.isUnderConstruction()))
                {
                    count++;
