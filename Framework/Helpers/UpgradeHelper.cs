...
namespace AnythingAnywhere.Framework.Helpers
{
    internal static class UpgradeHelper
    {
        ...
        {
            System.Collections.Generic.List<System.Collections.Generic.KeyValuePair<string, string>>? cabinPageNames = CabinUtility.GetCabinsToUpgrade();
        ...
        {
            StardewValley.Buildings.Building cabinBuilding = Game1.getLocationFromName(cabin).ParentBuilding;
            Cabin cabinInstance = (Cabin)cabinBuilding.indoors.Value;
        ...
        public static void CompleteHouseUpgrade(Farmer owner, bool debug = false)
        {
            ...
            if (debug)
            {
                string msg = $"Upgrading cabin at {Utility.getHomeOfFarmer(owner).NameOrUniqueName} from level {owner.HouseUpgradeLevel} to {owner.HouseUpgradeLevel + 1}";
                ModEntry.ModMonitor.Log(msg, LogLevel.Debug);
            }

            FarmHouse homeOfFarmer = Utility.getHomeOfFarmer(owner);
        ...
        {
            int count = 0;
            foreach (Cabin cabin in CabinUtility.GetCabins())
            {
                ...
                string msg = $"Upgrading cabin at {cabin.NameOrUniqueName} from level {cabin.owner.HouseUpgradeLevel} to 3";
                ModEntry.ModMonitor.Log(msg, LogLevel.Debug);

                FarmHouse homeOfFarmer = Utility.getHomeOfFarmer(cabin.owner);
